...
db = psycopg2.connect(host='localhost', user='postgres', password='Ram@1294', port=5432, database="phonepe")
cur = db.cursor()
...
#table 2
def agg_user_file(tpath, agg_user_list):
    ...
    for state in agg_user_list:
        ...
        for year in year_list:
            ...
            for data in Q_file:
                ...
                file_content = open(file_path, "r")
                file_data = json.load(file_content)
                if (file_data["data"]["userByDevice"] == None):
                    pass
                else:
                    try:
                        for i in file_data["data"]["userByDevice"]:
                            user_brand = i["brand"]
                            user_count = i["count"]
        ...
        for year in year_list:
            ...
            for data in Q_file:
                ...
                for i in file_data["data"]["districts"]:
                    ...
                    Dict_trans["Amount"].append(trans_amount)
                    Dict_trans["State"].append(state)
                    Dict_trans["Year"].append(int(data.strip('.json')))
...
def top_dist_trans(top_dist_trans_df):
    ...
    db.commit()
    cur.execute("CREATE TABLE top_dist_trans (State varchar(100), Year int, Quarter int, District varchar(100), Count int, Amount real)")
...
def top_pin_trans(top_dist_trans_df):
    cur.execute("DROP TABLE IF EXISTS top_pin_trans;")
    db.commit()
    cur.execute("CREATE TABLE top_pin_trans (State varchar(100), Year int, Quarter int, Pincode int, Count int, Amount real)")
        ...
        for year in year_list:
            ...
            for data in Q_file:
                ...
                for i in file_data["data"]["districts"]:
                    ...
                    reg_user_count = i["registeredUsers"]
                    Dict_user["District_Name"].append(user_dist_name)
                    Dict_user['RegisteredUsers'].append(reg_user_count)
                    Dict_user['State'].append(state)
                    Dict_user['Year'].append(year)
                    Dict_user['Quarter'].append(int(data.strip('.json')))
...
def top_dist_user(top_dist_user_df):
    ...
    db.commit()
    cur.execute("CREATE TABLE top_dist_user (State varchar(100), Year int, Quarter int, District varchar(100), Registered_User int)")
    for i, row in top_dist_user_df.iterrows():
        sql = "INSERT INTO top_dist_user VALUES (%s,%s,%s,%s,%s,%s)"
        cur.execute(sql, tuple(row))
        db.commit()
...
def top_pin_user(top_pin_user_df):
    cur.execute("DROP TABLE IF EXISTS top_pin_users;")
    db.commit()
    cur.execute("CREATE TABLE top_pin_user (State varchar(100), Year int, Quarter int, Pincode varchar(100), Registered_Users int)")
    for i, row in top_pin_user_df.iterrows():
        sql = "INSERT INTO top_pin_user VALUES (%s,%s,%s,%s,%s,%s)"
        cur.execute(sql, tuple(row))
        db.commit()
        
# 1.Aggregated transaction
aggre_trans_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\aggregated\\transaction\\country\\india\\state\\"
...
# 2.Aggregated User
aggre_user_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\aggregated\\user\\country\\india\\state\\"
...
# 3.Map Trans
map_trans_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\map\\transaction\\hover\\country\\india\\state\\"
...
# 4.Map User
map_user_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\map\\user\\hover\\country\\india\\state\\"
...
# 5top dist trans
top_dist_trans_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\top\\transaction\\country\\india\\state\\"
...
# 6top pincode trans
top_pin_trans_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\top\\transaction\\country\\india\\state\\"
...
# 7top dist user
top_dist_user_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\top\\user\\country\\india\\state\\"
...
# 8top pin user
top_pin_user_path = "D:\\Studies\\GUVI\\projects\\Phonepe\\pulse-master\\pulse-master\\data\\top\\user\\country\\india\\state\\"
...
with st.sidebar:
    opt = option_menu(
        menu_title="Menu",
        options=["Aggregated Analytics", "Top Analytics", "Insights"],
        icons="cast",
        default_index=0)

if opt == "Aggregated Analytics":
...
if search:
        ...
        if state == "All" and year == "All" and quarter == "All":
            ...
            a_t = cur.fetchall()
        elif state != "All" and quarter != "All" and quarter == "All":
            cur.execute(f"SELECT * FROM AGG_TRANS WHERE STATE = '{state}' AND YEAR = {year}")
            a_t = cur.fetchall()
        elif state == "All" and year != "All" and quarter != "All":
            cur.execute(f"SELECT * FROM AGG-TRANS WHERE YEAR = {year} AND QUARTER = {quarter}")
            a_t = cur.fetchall()
        else:
            cur.execute(f"SELECT * FROM AGG_TRANS WHERE STATE = '{state}' AND YEAR = {year} AND QUARTER = {quarter}")
            a_t = cur.fetchall()
    ...
    if page == "State" and res1:
        if year == "All" and quarter == "All":
            ...
            fig = px.pie(df, values="Total Amount", names="State", color="State",
                         title="Top 10 state-wise transaction for the last five years and all four quarters")
            st.plotly_chart(fig)
            
            cur.execute("SELECT STATE, SUM(REGISTERED_USERS) AS TOTAL_REG_USERS FROM TOP_DIST_USER GROUP BY STATE "
                        "ORDER BY TOTAL_REG_USER DESC LIMIT 10")
            ...
            fig_1 = px.pie(df_u, values="Reg_users", names="State", color="State",
                           title="Top 10 state-wise users for the last five years and all four quarters")
            ...
            df = pd.DataFrame(fetch, columns=["State", "Total Amount", "Total count"])
            fig = px.pie(df, values="Total Amount", names="State", color="State",
                         title=f"Top 10 state-wise transaction for {year} year and all four quarters")
            st.plotly_chart(fig)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["State", "Reg_users"])
            fig_1 = px.pie(df_u, values="Reg_users", names="State", color="State",
                           title=f"Top 10 state-wise users for {year} year and all four quarters")
            st.plotly_chart(fig_1)
            
        elif year == "All" and quarter != "All":
            cur.exec(f"SELECT STATE, SUM(AMOUNT) AS TOTAL_AMOUNT, SUM(COUNT) AS TOTAL_COUNT FROM TOP_DIST_TRANS "
                     f"WHERE QUARTER={quarter} GROUP BY STATE ORDER BY TOTAL_AMOUNT DESC LIMIT 10")
            fetch = cur.fetchall()
            df = pd.DataFrame(fetch, columns=["State", "Total Amount", "Total count"])
            fig = px.pie(df, values="Total Amount", names="State", color="State",
                         title=f"Top 10 state-wise transaction for all year and {quarter} quarter")
            st.plotly_chart(fig)
            ...
            fig_1 = px.pie(df_u, values="Reg Users", names="State", color="State",
                           title=f"Top 10 state-wise users for all years and {quarter} quarter")
            st.plotly_chart(fig_1)
            
        else:
            cur.execute(f"SELECT STATE, SUM(AMOUNT) AS TOTAL_AMOUNT, SUM(COUNT) AS TOTAL_COUNT FROM "
                        f"TOP_DIST_TRANS WHERE YEAR = {year} AND QUARTER={quarter} GROUP BY STATE ORDER BY TOTAL_AMOUNT DESC LIMIT 10")
            fetch = cur.fetchall()
            df = pd.DataFrame(fetch, columns=["State", "Total Amount", "Total count"])
            fig = px.pie(df, values="Total Amount", names="State", color="State",
                         title=f"Top 10 state-wise transaction for year {year} and quarter {quarter}")
            st.plotly_chart(fig)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["State", "Reg_users"])
            fig_1 = px.pie(df_u, values="Reg_users", names="State", color="State",
                           title=f"Top 10 state-wise users for year {year} and quarter {quarter}")
            st.plotly_chart(fig_1)
    ...
    if page == "Pincode" and res1:
        if year == "All" and quarter == "All":
            ...
            fig = px.pie(df, values="Total_Amount", names="Pincode",
                         title=f"Top 10 pincode-wise transaction for all the five years and four quarters")
            st.plotly_chart(fig)
            ...
            fig_u = px.pie(df_u, values="Total_reg_users", names="Pincode",
                           title=f"Top 10 pincode-wise transaction for all the five year and four quarters")
            ...
            df = pd.DataFrame(fetch, columns=["Pincode", "Total_Amount", "Total Count"])
            fig = px.pie(df, values="Total_Amount", names="Pincode", 
                         title=f"Top 10 pincode-wise transactions for {year} year ans all four quarters")
            st.plotly_chart(fig)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["Pincode", "Total_reg_users"])
            fig_u = px.pie(df_u, values="Total_reg_users", names="Pincode",
                           title=f"Top 10 pincode-wise transactions for {year} year and all four quarters")
            st.plotly_chart(fig_u)
            ...
            df = pd.DataFrame(fetch, columns=["Pincode", "Total_Amount", "Total Count"])
            fig = px.pie(df, values="Total_Amount", names="Pincode",
                         title=f"Top ten pincode-wise transactions for all the five years and quarters {quarter}")
            st.plotly_chart(fig)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["Pincode", "Total_reg_users"])
            fig_u = px.pie(df_u, values="Total_reg_users", names="Pincode", 
                           title=f"Top 10 pincode-wise transactions for all the five years and quarters {quarter}")
            st.plotly_chart(fig_u)
            ...
            df = pd.DataFrame(fetch, columns=["Pincode", "Total_Amount", "Total Count"])
            fig = px.pie(df, values="Total_Amount", names="Pincode", 
                         title=f"Top 10 pincode-wise transactions for year {year} and quarter {quarter}")
            st.plotly_chart(fig)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["Pincode", "Total_reg_users"])
            fig_u = px.pie(df_u, values="Total_reg_users", names="Pincode",
                           title=f"Top 10 pincode-wise transactions for year {year} ans quarter {quarter}")
            st.plotly_chart(fig_u)
    if page == "District" and res1:
        if year == "All" and quarter == "All":
            ...
            fig = px.pie(df, values="Total_Amount", names="District", 
                         title="Top 10 district-wise transactionsfor all five years and four quarters")
            st.plotly_chart(fig)
            ...
            fig_u = px.pie(df_u, values="Total Reg users", names="District",
                           title="Top 10 district-wise users for all the five years and four quarters")
            st.plotly_chart(fig_u)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["District", "Total Reg users"])
            fig_u = px.pie(df_u, values="Total Reg users", names="District",
                           title=f"Top 10 district-wise users for {year} year and all four quarters")
            st.plotly_chart(fig_u)
            ...
            df_u = pd.DataFrame(fetch_u, columns=["District", "Total Reg users"])
            fig_u = px.pie(df_u, values="Total Reg users", names="District", 
                           title=f"Top 10 district-wise users for all the five years and quarter {quarter}")
            st.plotly_chart(fig_u)
            
        else:
            cur.execute(f"SELECT DISTRICT, SUM(AMOUNT) AS TOTAL_AMOUNT, SUM(COUNT) AS TOTAL_COUNT FROM TOP_DIST_TRANS "
                        f"WHERE YEAR={year} AND QUARTER={quarter} GROUP BY DISTRICT ORDER BY TOTAL_AMOUNT DESC LIMIT 10")
            fetch = cur.fetchall()
            df = pd.DataFrame(fetch, columns=["District", "Total_Amount", "Total Count"])
            fig = px.pie(df, values="Total_Amount", names="District",
                         title=f"Top 10 district-wise transactions for year {year} and quarter {quarter}")
            st.plotly_chart(fig)
            
            cur.execute(f"SELCT DISTRICT, SUM(REGISTERED_USERS) AS TOTAL_REG_USERS FROM TOP_DIST_USER WHERE "
                        f"YEAR={year} AND QUARTER={quarter} GROUP BY DISTRICT ORDER BY TOTAL_REG_USERS DESC LIMIT 10")
            fetch_u = cur.fetchall()
            df_u = pd.DataFrame(fetch_u, columns=["District", "Total Reg users"])
            fig_u = px.pie(df_u, values="Total Reg users", names="District",
                           title=f"Top 10 district-wise users for {year} year and quarter {quarter}")
            st.plotly_chart(fig_u)
    ...
    if result:
        ...
        if state == "All" and year == "All" and quarter == "All":
            ...
            cur.execute(f"SELECT STATE, SUM(AMOUNT) AS TOTAL_AMOUNT, SUM(COUNT) AS TOTAL COUNT FROM "
            ...
            cur.execute(f"SELECT STATE, SUM(AMOUNT) AS TOTAL_AMOUNT, SUM(COUNT) AS TOTAL_COUNT FROM "
                        f"MAP_TRANS WHERE STATE='{state}' AND YEAR={year} AND QUARTER={quarter} GROUP BY STATE")
            fetch = cur.fetchall()
            
        df =pd.DataFrame(fetch, columns=["State", "Total Amount", "Total count"])
        df["State"] = df["State"].replace(state_name)
        fig = px.choropleth(df, geojson="https://gist.githubusercontent.com/jbrobst/56c13bbbf9d97d187fea01ca62ea5112"
